...
recommended_tracks_from_colour_with_title = pd.merge(recommended_tracks_from_colour,track_features_all_df[["title","artist_name","track_id"]],on='track_id', how='left')

print(recommended_tracks_from_colour)

# visualise the album artwork of the chosen songs
plt.style.use("dark_background")

plt.figure(figsize=(10, 10))

for i, track_id in enumerate(recommended_tracks_from_colour['track_id'].values):
    ax = plt.subplot(4, 3, i + 1)
    
    # get the image path
    img_path = [file for file, _ in albums if os.path.splitext(os.path.basename(file))[0] == track_id][0]
    
    # load them in 
    img = mpimg.imread(img_path)
    plt.imshow(img)
    
    # get the artist and title name
    title_artist = recommended_tracks_from_colour_with_title[
        recommended_tracks_from_colour_with_title['track_id'] == track_id
    ][['title', 'artist_name']].values[0]
    
    plt.title(f"{title_artist[0]} - {title_artist[1]}", fontsize=12)
    plt.axis("off")

# add in the input color
ax = plt.subplot(4, 3, len(recommended_tracks_from_colour) + 1)
plt.imshow(np.ones((10, 10, 3)) * np.array(input_colour) / 255)
plt.title("Input Color", fontsize=12)
plt.axis("off")
plt.suptitle("Recommended Tracks and Input Colour", fontsize=16)

# Save and show the figure
#plt.savefig("recommended_tracks_with_input_colour.png")
plt.show()
